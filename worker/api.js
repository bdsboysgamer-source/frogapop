// Leaderboard API — shared by the Worker entry (worker/index.js) and the
// Pages Function shim (functions/api). Framework-agnostic: takes a Request
// + env and returns a Response.
//
// Needs a D1 binding named DB. If DB is absent, /health reports ok:false
// and the client stays on local boards — so the app works with no setup.
...
export async function handleApi(request, env) {
  ...
  try {
    ...
    if (route === 'account' && request.method === 'POST') {
      ...
      await db.prepare('INSERT INTO users (name, token, created) VALUES (?, ?, ?)').bind(name, token, Date.now()).run();
      return json({ ok: true, name, token });
    }

    if (route === 'score' && request.method === 'POST') {
      const { token, key, score } = await request.json();
      const s = Math.max(0, Math.min(1e9, Math.floor(Number(score) || 0)));
      if (!token || !key) return json({ ok: false, error: 'bad-req' }, 400);
      const user = await db.prepare('SELECT id, name FROM users WHERE token = ?').bind(token).first();
